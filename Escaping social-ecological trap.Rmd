...
#We identified the final one breakpoint 1981 and divided two periods: 1949-1981 (32 years) and 1981-2020 (38 years)
#The difference in the length of two time periods is very small.
...
#现在不是分类的，而是开始的是将一些的边的重新分类，还有就是如何将这邪边的进行重新地方二类的
#还有就是还不是时候的的的的的的对的
#还有就是现在的是的，现在数的的的
#现在需要将数据的分为1，3，5，7，9
...
Gini_coef = read_xlsx("Gini coefficient.xlsx")
Forest_cover = read_xlsx("Forest_cover.xlsx")
head(data_elements)
...
p15<-ggplot(data=Gini_coef) +
  annotate("rect",xmin=2003, xmax=Inf, ymin=-Inf, ymax=Inf,fill='#8ECFC9',alpha = 0.7)+
  geom_line(aes(x=Year, y=Shanxi),color="#e95280", size=1) +
  geom_line(aes(x=Year, y=Shaanxi),color="#3fbac2", size=1) +
